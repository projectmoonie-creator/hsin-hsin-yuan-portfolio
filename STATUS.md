...
- Package: `Women Make Waves Press Notes v1`
...
- Review:
  `docs/reviews/women-make-waves-press-notes-v1-2026-08-03.md`
- Latest Vercel Preview:
  `https://hsin-hsin-yuan-portfolio-preview-7kgh2kg8o.vercel.app`
- Deployment ID: `dpl_A3pbY9xXZqSjbBh3NN11eS3C7QBv`
...
This package adds a low-priority `Press & Conversations` / `訪談與對談`
section after Archive and before Contact. It preserves two Women Make Waves
Film Festival selection-guide videos as public Facebook links without adding
them to Featured Works. The official upper-half record verifies `袁欣欣` as a
speaker; the lower-half link is labelled only as a companion event archive.
The section is text-only because logged-out Facebook metadata supplied no
reliable video-specific image.

## Verification

- TDD: three press data, rendering, and layout tests failed against the old
...
- Local browser QA passed at `1440 × 900`, `1200 × 900`, `834 × 1112`,
  `390 × 844`, and `360 × 800`, plus reduced-motion and no-JavaScript states:
  two text-only equal-height cards, correct section order, visible focus, no
...
1. Manually open the latest Preview and inspect the Press section and both
   Facebook links. Do not submit the contact form.
...
3. Facebook remains a public third-party availability and logged-out access
   risk. The page does not depend on its images or metadata for layout.
4. Future interviews belong in `data/press.json` after title, participation,
   public-link, and image-rights verification.
