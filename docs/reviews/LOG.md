...
- Integration/Preview: local `main` reaches deploy source `b7d3fe1`; adopted dist-only deployment `dpl_Gy96AtLGzKzLyAwQEzVhwbtUw7Rz` is `READY` / Preview at `https://hsin-hsin-yuan-portfolio-ku5d9znnk.vercel.app`; no Production, alias, `origin/main`, or Contact action.
