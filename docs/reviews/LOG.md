...
- Pre-release boundary: protected document remained untracked at `945d4df9…d28fc`; no push, `main`, Contact, video, copy, layout, or unrelated media change.
...
- Production: producer ruled `核准 Production、變更正式網站`; isolated 59-file / 96,783,855-byte package with unchanged Contact deployed as `dpl_5h1n…G4XQ`, read back Ready/Production with canonical alias and Contact function. Previous `dpl_7qt7…u3vn` is the rollback identity; no Git push, `main`, tag, Contact request, or protected-file action.
- Post-publish: PageSpeed mobile English reports 73/100/100/100, FCP 1.0s, LCP 9.4s, TBT 0ms, CLS 0, Speed Index 4.3s, 12,511 KiB, and no CrUX. It again reports both Slow Steps sources; the closed nine-run local package observed zero dual-source runs, so this single lab signal is recorded without reopening runtime work.
- Verdict/next: live release is Ready; overall `PASS_WITH_OPEN_ITEMS` only because the release source's six commits plus this record remain absent from origin. Full record: `hero-cover-production-release-2026-08-13.md`. Stay in maintenance mode and wait for separate Git/durability or new-package authorization.

## 2026-08-13 — Post-release Contact blocker correction
- Producer finding: own domain and professional contact-address/forwarding design were not surfaced before Production; the omission is accepted and recorded in new cold-start `PRODUCT-WISHES.md`.
- Read-only truth: Production has no Vercel environment variables, while `api/contact.js` requires `RESEND_API_KEY`, `CONTACT_TO_EMAIL`, and `CONTACT_FROM_EMAIL`; a real inquiry would therefore return the configured 503 response. No secret was read and no Contact/email action occurred.
- Adjudication: the Vercel project domain is an accepted temporary public origin under the Bible, but the unavailable primary inquiry route is a functional launch blocker. Overall status changes to `BLOCKED`; the already-Ready Hero/site deployment is not rolled back because rollback has the same mail-configuration uncertainty and would not resolve the cause.
- Next: one bounded decision package for domain, public alias, private forwarding destination, authenticated sender, DNS/provider ownership, environment setup, failure fallback, and a consented receive/reply test. No purchase, DNS, alias, Production, environment, Contact, email, Git push, `main`, tag, or protected-file action is authorized yet.

## 2026-08-13 — Custom-domain Production cutover
- Authorization/scope: producer purchased `hsinhsinyuan.com`, approved connecting the formal portfolio, and approved the frozen-packet Gemini API review; Contact/mail, Git push, `main`, tag, and destructive Vercel actions stayed out of scope.
- Candidate: local `55658db0284aa9f295ecb88da1320f26a9bccf1c` changes only canonical origin plus active Figma URL/allowlist. TDD RED 3 expected failures, then focused 49/49 and full 179/179 GREEN; build, Figma, audit, integrity, privacy, dependency, diff, and 12-case browser gates pass.
- Review: packet-at-review `6c3b0fa…0aaf5`; Gemini requested/observed/completed `gemini-3.6-flash`, PASS/no findings and cutover approved. Claude attempt `45ef1139…6500` is incomplete with no observed/completed model; no dual consensus. After the privacy gate failed, only the private checkout label and provisional full SHA were corrected; current packet `b1fa4f1…902e`, with both hashes retained in adjudication.
- Package: isolated 59 files / 96,783,418 bytes / path-list `4a35f046…8c7`; exact `dist/`, unchanged Contact, minimal config, no source/private/protected payload.
