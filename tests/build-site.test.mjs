...
test("all approved Featured reels hold their existing poster before muted playback", () => {
  ...
  assert.match(
    html,
    /<article class="work-panel" id="tech-dreamers">[\s\S]*?<a class="media-frame media-frame-reel-switch media-frame-unlabeled media-frame-link" href="https:\/\/www\.taiwanplus\.com\/shows\/documentary\/business-and-tech\/590\/tech-dreamers" target="_blank" rel="noreferrer" aria-label="Play video: Tech Dreamers"[\s\S]*?data-featured-reel-video/,
  );
  assert.match(
    html,
    /<article class="work-panel" id="my-art-my-voice">[\s\S]*?<a class="media-frame media-frame-reel-switch media-frame-unlabeled media-frame-link" href="https:\/\/www\.taiwanplus\.com\/shows\/documentary\/arts\/410\/my-art-my-voice\/250220001\/whats-the-vibe-in-taiwan-my-art-my-voice" target="_blank" rel="noreferrer" aria-label="Play video: My Art, My Voice"[\s\S]*?data-featured-reel-video/,
  );

  const slowStepsPanel = html.match(
    /<article class="work-panel" id="slow-steps">([\s\S]*?)<\/article>/,
  )?.[1] || "";
  assert.match(slowStepsPanel, /<div class="media-frame media-frame-reel-switch media-frame-unlabeled"/);
...
test("Featured reels keep approved poster and playing geometries", () => {
  const site = loadSiteData(root);
  const works = loadWorks(join(root, "content/works"));
  const html = renderPage({ lang: "en", site, works });
  const css = readFileSync(join(root, "src/styles.css"), "utf8");
  ...
  for (const work of approved) {
    const panel = html.match(
      new RegExp(`<article class="[^"]+" id="${work.slug}">[\\s\\S]*?<\\/article>`),
    )?.[0] || "";
    assert.match(panel, /data-featured-reel-video/);
    if (switching.has(work.slug)) {
      assert.equal(work.presentation.desktopMediaVariant, "fill-poster-16x9-reel");
      assert.match(panel, /<article class="work-panel"/);
      assert.match(panel, /class="media-frame media-frame-reel-switch(?: [^"]*)?"/);
      assert.doesNotMatch(panel, /media-frame-wide/);
    } else {
      assert.equal(work.presentation.desktopMediaVariant, "centered-16x9");
      assert.match(panel, /<article class="work-panel work-panel-wide-media"/);
  ...
  assert.match(
    js,
    /function resetFeaturedReel\(video\) \{[\s\S]*?video\.closest\("\.media-frame"\)\?\.classList\.remove\("is-reel-playing"\);/,
  );
  assert.match(
    css,
    /\.media-frame-reel-switch\.is-reel-playing \{[\s\S]*?align-self: center;[\s\S]*?aspect-ratio: 16 \/ 9;[\s\S]*?min-height: 0;[\s\S]*?width: 100%;/,
  );
  assert.match(
    css,
    /@media \(max-width: 820px\) \{[\s\S]*?\.media-frame \{[\s\S]*?aspect-ratio: 16 \/ 9;[\s\S]*?min-height: 0;/,
  );
  assert.doesNotMatch(css, /#(?:slow-steps|tech-dreamers|my-art-my-voice|top-gear-china-uk-special)/);
...
test("renderPage creates bilingual page with scroll-stack works and video fallbacks", () => {
  ...
  assert.match(html, /<link rel="preload" as="image" type="image\/avif" href="\/assets\/portfolio\/hero\/hsin-working-white-space-1440\.avif" imagesrcset="[^"]*960\.avif 960w, [^"]*1440\.avif 1440w" imagesizes="\(min-width: 1281px\) 47vw, 90vw" media="\(min-width: 821px\)" fetchpriority="high">/);
  ...
  assert.match(html, /<source type="image\/webp" media="\(min-width: 821px\)" srcset="[^"]*960\.webp 960w, [^"]*1440\.webp 1440w" sizes="\(min-width: 1281px\) 47vw, 90vw">/);
  assert.match(html, /<source type="image\/jpeg" media="\(max-width: 820px\)" srcset="[^"]*640\.jpg 640w, [^"]*960\.jpg 960w" sizes="90vw">/);
  assert.match(html, /<img class="hero-media-image" src="\/assets\/portfolio\/hsin-working-white-space\.jpg" width="1448" height="1086" alt="Hsin-Hsin Yuan working on a laptop in a bright white studio" loading="eager" decoding="sync" fetchpriority="high">/);
  ...
  assert.match(html, /data-hero-width="1448" data-hero-height="1086"/);
  ...
  assert.match(
    html,
    /<article class="work-panel" id="slow-steps">[\s\S]*?class="media-frame media-frame-reel-switch media-frame-unlabeled" style="background-image:[\s\S]*?slow-steps-poster\.webp[\s\S]*?<\/div>[\s\S]*?<div class="work-copy">/,
  );
  assert.doesNotMatch(
    html,
  ...
  assert.match(
    html,
    /class="media-frame media-frame-reel-switch media-frame-unlabeled media-frame-link"[\s\S]*?href="https:\/\/www\.taiwanplus\.com\/shows\/documentary\/business-and-tech\/590\/tech-dreamers"[\s\S]*?aria-label="Play video: Tech Dreamers"[\s\S]*?background-image:[\s\S]*?224be7ed-057b-400f-af63-a8582cd80cfb\.webp/,
  );
  assert.match(
    html,
    /class="media-frame media-frame-reel-switch media-frame-unlabeled media-frame-link"[\s\S]*?href="https:\/\/www\.taiwanplus\.com\/shows\/documentary\/arts\/410\/my-art-my-voice\/250220001\/whats-the-vibe-in-taiwan-my-art-my-voice"[\s\S]*?aria-label="Play video: My Art, My Voice"/,
  );
